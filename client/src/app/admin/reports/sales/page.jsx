// client/src/app/admin/reports/sales/page.js
...
export default async function Page() {
  ...
  // Default sort=desc; append your filters if needed
  const res = await fetch(`${API_ORIGIN}/api/public/reports/sales?sort=desc`, {
  ...
  return (
    <main className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold text-root-primary">Sales Report</h1>
      <div className="rounded-2xl bg-surface border border-root overflow-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td className="p-3 text-root-secondary" colSpan={4}>No data.</td></tr>
            ) : (
              rows.map(r => (
